...
- `packages/lead-design-tokens-cli/` contains the Lead token pipeline: import, normalize, lint, build, decision checks, schemas, and authored token inputs. The package also hosts the Button-only Lane 3 advisory audit prototype at [`packages/lead-design-tokens-cli/docs/audits/`](packages/lead-design-tokens-cli/docs/audits/) (JES-77; read-only, no Figma API, no CI integration).
